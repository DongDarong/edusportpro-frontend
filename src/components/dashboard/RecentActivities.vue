<template>
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 md:p-6 col-span-1">
    <h2 class="text-lg font-bold text-hope-dark mb-5 flex items-center gap-2">
      <span class="w-2 h-2 rounded-full bg-hope-yellow"></span>
      Recent Activity
    </h2>

    <p v-if="loading" class="text-sm text-gray-500">Loading recent activity...</p>
    <p v-else-if="error" class="text-sm text-red-700">{{ error }}</p>

    <ul v-else class="relative border-l border-gray-200 ml-2 space-y-6">
      <li v-for="activity in activities" :key="activity.id" class="ml-6">
        ...
        ></span>
        <p class="text-sm font-medium text-hope-dark">{{ activity.message }}</p>
        <p class="text-xs text-gray-500">{{ activity.time }}</p>
      </li>
      <li v-if="!activities.length" class="ml-6">
