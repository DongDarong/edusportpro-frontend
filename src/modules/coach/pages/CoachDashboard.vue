...
async function loadDashboard() {
  ...
  try {
    ...
    loadError.value = error instanceof Error ? error.message : 'Unable to load coach dashboard data.'
  } finally {
    loading.value = false
  }
}
  ...
  <MainLayout>
    ...
    <section class="dashboard">
      <div v-if="loading" class="dashboard__state">
        <LoadingSpinner label="Loading coach data" size="md" />
      </div>
      ...
      <template v-else>
        ...
        <article class="dashboard__panel">
          <header class="dashboard__panel-head">
            ...
            <StatusBadge status="info" label="Today" :dot="true" size="sm" />
          </header>

          <ul class="dashboard__activity">
            <li v-for="session in upcomingSessions" :key="session.id" class="dashboard__activity-item">
              <div>
                <p class="dashboard__activity-message">{{ session.title }}</p>
                <p class="dashboard__activity-time">{{ session.time }} - {{ session.location }}</p>
              </div>
